...
class PowerBIProcessor(FileProcessor):
    def __init__(self,zip_file):
        ...
        if not zipfile.is_zipfile(self.zip_file):
            raise "Please upload valid zip file"
    def process(self):
        ...
        try:
            ...
            st.warning("ER diagram was not able to generate at the moment. Please try again after sometime.")
...
def markdown_to_docx(markdown_text):
    ...
    try:
        docx_data = pypandoc.convert_text(markdown_text, 'docx', format='md')
        return docx_data.encode('latin1')  # Convert string to bytes for download
    except Exception as e:
        st.warning(f"Error converting to DOCX: {e}")
        return None
def main():
    load_dotenv()
